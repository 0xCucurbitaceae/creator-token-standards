...
contract TransferValidatorTestERC1155 is TransferValidatorTest {
    ...
    function testAddAccountsToRecipientAllowlist(address listOwner, uint256 numAccountsToWhitelist, address[10] memory accounts) public {
        ...
        CreatorTokenTransferValidator creatorTokenTransferValidator = CreatorTokenTransferValidator(address(validator));
        ...
        for (uint256 i = 0; i < numAccountsToWhitelist; i++) {
            ...
            if (firstTimeAccount) {
                ...
                emit AddedAccountToList(LIST_TYPE_RECIPIENTS, listId, accounts[i]);
            }
        }

        vm.prank(listOwner);
        creatorTokenTransferValidator.addAccountsToRecipientAllowlist(listId, accountsToWhitelist);
